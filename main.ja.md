...
要するに **「情報流の波導化 → 1/r ドリフト → ゼロ自由度の観測比」** という鎖こそが FDB のビッグピクチャーであり、本書はこの鎖の物理と検証法を順番にひも解いていく。
...
理論の詳細は §6–§9、観測指標は §10（H1・BTFR）を参照すると最短経路で把握できる。
...
一般相対論 (GR) は「質量・エネルギーが時空を曲げ、物質は曲率の最短経路 (測地線) に沿って運動する」という幾何学的描像を採る。FDB は同じ方程式体系を背景に持ちつつ、「情報流の偏りが十分に小さい領域では GR と同じ結果を返し、銀河–ボイド界面のように勾配が大きくなると 1/r ドリフトが立ち上がる」という補完的視点を提示する。
...
FDB では重力を基本相互作用リストから外し、電磁・弱・強の 3 作用と **情報流ドリフト**の組み合わせで宇宙を描く視点を採る。重力は Proca 場の巨視的統計揺らぎが生む**有効力**とみなし、GR はその短距離極限を表す理論として併存させる。
...
再現の最短手順は以下のとおりである。追加のオプションやログの整形は README を参照すればよい。
...
| 用語 | 意味 |
|---|---|
| ULE-EM | 超低エネルギーの Proca 電磁場。フォトン質量は \(m_\gamma\sim3.3\times10^{-64}\,\mathrm{kg}\)（本稿の推定値）。|
