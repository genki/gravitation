...
Future Determination Bias (FDB) は、**情報流束**を用いたゼロ自由度の等式を中心に構成される:
...
ここで 1 をボイド側、2 を銀河側とすると、$k_{z,1}^2>0$・$k_{z,2}^2 < 0$ となる周波数帯で高反射 ($|R|pprox 1$) が実現する。実効遮蔽長 $\delta = 1/|\mathrm{Im}\,k_{z,2}|$ は 10–30 kpc 程度で、銀河スケールより短い。

### 3.2 幾何学的帰結

波が界面に沿って伝わると、球面幾何から円筒幾何に変わり、エネルギー流束保存は
$$
2\pi r I(r) = \text{const} \Rightarrow I(r) \propto \frac{1}{r}
$$
となる。情報流束 $I(r)$ が 1/r で減衰すると、後述する確率ドリフトを通じて**有効力も 1/r** になる。
...
- **強重力**: 近接ブラックホール周りでは導波路条件が破れ、3D 逆二乗に戻る。GR との厳密整合は今後の課題。
