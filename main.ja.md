...
> **読み方のナビ** — まず **§3「観点の再整理」**（GR 極限と用語ミニ枠）→ **図1（幾何とドリフトの対比）** → **§11（H1 と BTFR の即時検証）** の順で読み、その後に **§7–§10（Proca 場・波導・情報束）** で数理背景を深掘りするのが最短経路。
...
短結論: 一様な情報監視率（\(\nabla\Gamma=0\)）の領域では FDB の加速度は消え、GR/SIS と完全に同型。外縁だけ 1/r 力が立ち上がる（太陽系・連星パルサ・重力波源と矛盾しない）。
