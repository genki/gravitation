...
- Method and numbers are summarized in `appendix_f_h1.md`; code at `src/analysis/h1_ratio_test.py`, data under `data/strong_lensing/`; figure `figures/h1_violin.png`.

---

## Data and code availability
Derived figures and tables come from the scripts distributed with this repository. `src/analysis/h1_ratio_test.py` regenerates Table 1 and Figure \ref{fig:h1-violin} from the SLACS/BELLS/BELLS GALLERY/S4TM/BOSS catalogs cited above, while `src/analysis/sparc_fit_light.py` and `src/scripts/sparc_sweep.py` reproduce Table 2, Figure \ref{fig:rotcurve-grid}, and Figure \ref{fig:btfr} directly from the SPARC MRT release. All intermediate CSV outputs live under `build/` and the raw catalogs sit in `data/strong_lensing/` and `data/sparc/`, enabling a full rerun without additional downloads.
