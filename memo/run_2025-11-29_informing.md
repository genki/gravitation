...
- 11/30: v2 を NGC2403/3198/6503/ DDO170/168 に適用。代表値:
  ...
  - その後 dR_est 由来の狭すぎる殻厚が W(R) を急峻化しすぎていることが判明し、殻厚は銀河共通のほぼ定数として扱う方針に修正。
- 11/30: v2 カーネルで殻厚 dR をグローバル定数 (D_R_CONST_KPC ~1kpc) に固定。R_t は Σ_gas ではなく「バルジ端＋ κ R_d」で定義する設計に更新:
  - Rd: Σ_star の指数フィットから決定
  - R_bulge_edge: Vbul/Vtot>0.3 の最外半径（なければ 0）
  - R_t(g) = R_bulge_edge(g) + κ R_d(g), κ∈[1,5] をフィット
  - χ² は R>3R_d の点のみで評価（星ディスク外側に限定）
- 11/30: 上記の新しい v2 定義で NGC2403/3198/6503/DDO170/168 を再フィット:
  - NGC2403: Rd≈1.36kpc, R_bulge_edge≈0 → κ≈3.87, R_t≈5.3kpc, Δv²≈1.43e4, ε=0.5, χ²_v2≈15.0
  - NGC3198: Rd≈3.22kpc, R_bulge_edge≈0 → κ≈3.06, R_t≈9.9kpc≈3R_d, Δv²≈1.80e4, ε=0.5, χ²_v2≈2.3
  - NGC6503: Rd≈1.39kpc, R_bulge_edge≈0 → κ≈4.00, R_t≈5.6kpc, Δv²≈1.09e4, ε=0.5, χ²_v2≈4.2
  - DDO170: Rd≈1.96kpc, R_bulge_edge≈0 → κ≈2.89, R_t≈5.7kpc, Δv²≈3.11e3, ε=0.5, χ²_v2≈0.18
  - DDO168: Rd≈1.07kpc, R_bulge_edge≈0 → κ≈1.00, R_t≈1.1kpc, Δv²≈2.19e3, ε≈0.086, χ²_v2≈7.3e-2
  → R_t は「バルジ端＋数R_d」という形で L* スパイラルと dwarf の両方で整合的に揃い、W(R) はバルジ内に入らない滑らかな立ち上がりに。Σ_gas の局所的勾配に振り回されず、FDB が想定する“殻＝バルジ外縁＋ディスク肩の少し外側”という幾何に近づいた。

## 2025-12-01 v2 カーネル（幾何混合）アップデート

- v2 カーネルを「情報量保存の凸結合」として再定義:
  - 以前: `v_tot^2 = v_Newt^2 (1 - eps W) + Δv² W`
  - 現在: `v_tot^2(R) = (1 - W(R)) v_Newt^2(R) + W(R) Vflat^2`  
    → W(R) が 0→1 になるにつれ、1/r² から 1/r に「重み」を再配分する構造に。
- W(R) の定義を R_t 由来の二重スイッチから幾何カップリング一本化へ:
  - 新しい幾何カップリング関数
    \[
      f_{\rm geom}(R) = \frac{L(R)/\lambda_C}{1 + L(R)/\lambda_C},\quad L(R)=\max(R,R_d)
    \]
    - R≪λ_C → f_geom≃0（1/r² 優勢）
    - R∼λ_C → f_geom∼0.5
    - R≫λ_C → f_geom→1（1/r 優勢）
  - R_t, κ による追加スイッチは廃止し、W(R)=f_geom(R/λ_C) に一本化。
  - Rd は Σ_star の指数フィットから推定し、λ_C は代表値 ~30 kpc を採用。
- gas 構造の扱い:
  - ガスは等高面を細分化し得るので `gas_smoothing_factor(Sigma_gas)` を導入したが、  
    1/r への再配分優先の方針のため混合には掛けず、プロット下段で参考として表示のみ（今後のチューニング余地）。
  - Σ_gas のプロット正規化は銀河ごと最大値→**全銀河共通最大値**に変更し、全 17 銀河でガスの厚い/薄い領域を同一スケールで比較できるようにした。
- カーネルの「物理的意味付け」の整理:
  - f_geom は「構造スケール L(R) と λ_C の比から決まるカップリング」であり、プラズマ密度 n_e や ω_p は殻に結合した後のローカルなスキンデプス評価に限定。
  - inner shell / outer shell の二層構造は、実装上は
    - inner shell: L∼R_d（ディスク外縁あたり）で f_geom が立ち上がる部分、
    - outer shell: L∼L_outer（CGM/IGM）のスケールに現れるほぼ一定オフセット（v0）的成分
    として扱う方針に再整理。
...
- 外部銀河（SPARC）:
  - L\*〜中質量スパイラル（10本）
    - NGC 2403, 2903, 3198, 3521, 5005, 5055, 6503, 7331, 7793, **1090**（新規）
  - dwarf / low-mass（7本）
    - NGC 2366, 3109, DDO 064, 161, 168, 170, KK98‑251
- Milky Way:
  - McGaugh の MilkyWayModel/MW2018modelWSD を SPARC 互換 CSV に変換した `build/MW_sparc.csv` をテスト用に利用。

### 数値的所見（抜粋）

- 幾何一本化後の v2 では、外縁 (R>3R_d) の χ² は旧 v2 より若干悪化する銀河もあるが、
  - 1/r²→1/r の再配分が明示的になり、
  - R≳λ_C の領域で v_tot が必ず Newton より 1/r 側に寄る挙動が保証される形になった。
- ガス量や内部バルジ問題（rotmod 側で Newton が V_obs を大きく上回るケース）は、  
  現段階では FDB カーネルの射程外（質量モデル側の系統誤差）として割り切っている。
