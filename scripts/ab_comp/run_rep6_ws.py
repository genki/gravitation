...
def fit_mu_alpha(g_obs: np.ndarray, eg: np.ndarray, g_gas: np.ndarray, g_star0: np.ndarray, g_extra: np.ndarray,
                 mask: np.ndarray) -> Tuple[float, float, float]:
    mb = mask & np.isfinite(g_extra)
    if not np.any(mb):
        return 0.7, 0.0, float('nan')
    ...
    chi = float(np.nansum((w**2) * ((g_gas[mb] + mu*X1 + alpha*X2 - g_obs[mb])**2)))
    return float(mu), float(alpha), chi
...
def main() -> None:
    if os.environ.get('GRAV_BG_JOB') != '1':
        sys.stderr.write(
            ...
            "scripts/jobs/dispatch_bg.sh -n rep6_ab_fast -- 'PYTHONPATH=. ./.venv/bin/python scripts/ab_comp/run_rep6_ws.py --fast'\n"
        )
        raise SystemExit(1)
    ap = argparse.ArgumentParser(description='rep6 — W·S(界面) representative table under fair settings')
    ...
    ap.add_argument('--out', default='data/results/rep6_ws_fast.json')
    # WS kernel grid: map s[kpc] to line_eps_kpc; use same values as Phi·eta s-grid
    ap.add_argument('--s', nargs='+', type=float, default=[0.4, 0.6, 1.0])
    ap.add_argument('--workers', type=int, default=0, help='parallel workers (0=auto when --workers-auto, 1=sequential)')
    ap.add_argument('--workers-auto', action='store_true', help='use ~60% of available CPUs when workers=0')
    args = ap.parse_args()

    # fixed RNG for reproducibility (even if deterministic path)
    try:
    ...
    if args.full:
        ...
        args.downsample = 1
        args.float32 = False
        args.out = 'data/results/rep6_ws_full.json'
    ...
    def _one(nm_s: Tuple[str, int]) -> Dict[str, Any]:
        ...
        try:
            ...
            best = None
            for s_kpc in args.s:
                g_ws = line_bias_accel(R, rc.SBdisk, pix_kpc=0.2, size=256, line_eps_kpc=float(s_kpc), pad_factor=2)
                mask = np.isfinite(g_obs) & np.isfinite(eg) & np.isfinite(g_gas) & np.isfinite(g_star0) & np.isfinite(g_ws)
                mu, a, chi = fit_mu_alpha(g_obs, eg, g_gas, g_star0, g_ws, mask)
                N = int(np.sum(mask))
                aic = aicc(chi, k=args.k, N=N)
                row = (aic, chi, mu, a, s_kpc, N)
                if best is None or row[0] < best[0]:
                    best = row
            if best is None:
                return {'name': nm, 'error': 'WS failed'}
            aic, chi, mu, a, s_kpc, N = best
            return {
                'name': nm,
                'N': int(N),
                'k': int(args.k),
    ...
    # parallel map over galaxies respecting ~60% CPU when requested
    ...
    # reconstruct lightweight command string for footnote
    try:
        rel = Path(__file__).relative_to(Path(__file__).parents[2])
        cmd = 'PYTHONPATH=. python ' + str(rel) + ' ' + ' '.join([h for h in sys.argv[1:]])
    except Exception:
        cmd = 'python run_rep6_ws.py ' + ' '.join([h for h in sys.argv[1:]])
    # compute sha of fairness/shared files and git rev
    ...
    meta = {
        ...
        'grid_s': [float(x) for x in args.s],
        'cmd': cmd,
        # use sha256 (12 chars) to align with other pages/footers
