...
def main() -> int:
    use_jp_font()
    ap = argparse.ArgumentParser(description='Velocity-field residual (ring-median) over Halpha contours — NGC2403')
    ap.add_argument('--vel', type=Path, default=Path('data/vel/NGC2403/velocity.fits'))
    ...
    vmax = np.nanpercentile(abs(resid[np.isfinite(resid)]), 95)
    im = ax.imshow(resid, origin='lower', cmap='coolwarm', vmin=-vmax, vmax=vmax)
    if ha_img is not None:
        ...
        if m.any():
            ...
            ax.contour(ha_img, levels=lv, colors=['#99c', '#66a', '#338', '#115'], linewidths=[0.6,0.9,1.2,1.5])
    ax.set_title('NGC 2403 — 面内残差（リング中央値差し引き）× Hα 等高線')
    ax.set_xticks([]); ax.set_yticks([])
    cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label('V_residual (arb.)')
    out = Path('server/public/reports/ngc2403_vfield_residual_ha.png')
