...
if [[ ${BASH_SOURCE[0]} != "$0" ]]; then
  echo "Do not source; run ./scripts/jobs/batch_w_eff_knee.sh" >&2
...
BASE_CMD="PYTHONPATH=. ./.venv/bin/python scripts/reports/make_bullet_holdout.py --train $TRAIN --holdout $HOLDOUT --band 8-16 --perm-n 5000 --perm-min 5000 --perm-max 5000 --block-pix 6"

# Compose a single sequential command string to run under dispatcher
CMD_STR="set -e; echo 'A-3 knee grid start: $HOLDOUT'"$'\n'
for q in "${Q_LIST[@]}"; do
  for p in "${P_LIST[@]}"; do
...
CMD_STR+=$'echo "A-3 knee grid done: '$HOLDOUT'"\n'

scripts/jobs/dispatch_bg.sh -n "w_eff_knee_${HOLDOUT}" --cwd "$REPO_ROOT" --env "$COMMON_ENV" --scope -- "$CMD_STR"
